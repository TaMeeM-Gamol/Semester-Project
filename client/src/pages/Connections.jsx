...
import {Users, UserPlus, UserCheck, UserRoundPen, MessageSquare, Heart} from 'lucide-react'
      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        {/* {label} */}
          ...
          <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
              {dataArray.map((tap)=>(
               <button onClick={()=> setCurrentTab(tap.label)} key={tap.label} className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors 
               ${currentTab ===tap.label ? 'bg-white font-medium text-black': 'text-gray-500 hover:text-black'}`}>
                 <tap.icon className='w-4 h-4'/>
                 <span className='ml-1'>{tap.label}</span>
                 {tap.count !==undefined && (<span className='ml-2 text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full'>{tap.count}</span>)}
               </button>
              ))}
          </div>
           ...
           <div className='flex flex-wrap gap-6 mt-6'>
            ...
            <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
              ...
              <div>
                ...
                <p className='text-sm text-gray-600'>{user.bio.slice(0,35)}...</p>
