...
const CreatePost = () => {
  ...
  const handleSubmit = async () => {
    if (!images.length && !content) {
      return toast.error('please add at least one image or post')
    }
    setLoading(true)
             ...
             <div className='flex items-center justify-between pt-3 border-t border-gray-300'>
               <label htmlFor="images" className='flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 transition cursor-pointer'>
                ...
                <button disabled={loading} onClick={()=> toast.promise(
                  ...
                  active:scale-95 transition text-white font-medium px-8 py-2 rounded-md cursor-pointer'>Puplish post</button>
             </div>
           </div>
      </div>
    </div>
