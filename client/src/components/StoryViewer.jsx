...
const StoryViewer = ({viewStory,setViewStory}) => {
    ...
    const renderContent= ()=>{
        switch (viewStory.media_type) {
            case 'image':
                ...
                case 'video':
                return(
                    <video onEnded={()=>setViewStory(null)} src={viewStory.media_url} alt="" className='pl-75 w-full h-screen flex items-center justify-center' controls autoPlay />
                );
                case 'text':
                return(
  ...
  return (
    <div className='fixed inset-0 h-screen bg-black bg-opacity-90 z-110 flex items-center justify' 
          ...
          <div className='absolute top-4 left-4 flex items-center space-x-3 
            p-2 px-4 sm:p-4 sm:px-8 backdrop-blur-2xl rounded bg-black/50'>
             ...
             <img src={viewStory.user?.profile_picture} alt="" className='size-7 sm:size-8 rounded-full object-cover border border-white'/>
              ...
              {/* {close button} */}
              <button onClick={handleClose} className='absolute top-4 right-2 text-white text-3xl font-bold focus:outline-none'>
