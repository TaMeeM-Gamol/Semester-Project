  ...
  return (
    <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md'>
      ...
      <div className='flex items-center justify-center gap-2 mt-4 text-xs text-gray-600'>
        ...
        <div className='flex items-center gap-1 border border-gray-300 rounded-full px-3 py-1'>
            <span>{user.followers?.length || 0}</span> Follwers 
        </div>
      </div>
      ...
      <div className='flex mt-4 gap-2'>
        ...
        {/* {connection reguest button / message button} */}
        <button onClick={handleConnectionRequests} className='flex items-center justify-center w-16 border text-slate-500 group rounded-md cursor-pointer active:scale-95 transition'>
            {
